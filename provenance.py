...
CLAIMS: tuple[Claim, ...] = (
    ...
    Claim(
        id="P1-E7f", layer=2,
        statement="PARTIAL 2026-09-25. On a SECOND family (Gemma-2-9B-it) the attack/control "
                  "decomposition replicates on four of five pre-registered criteria in all "
                  "three matched seeds -- mapping degraded (re-fit -1.44 / -1.56 / -1.06 "
                  "against controls +5.87 / +5.03 / +4.08), readout intact (frozen +5.09 / "
                  "+5.40 / +5.71), representation intact (probe 1.000 at every dose), "
                  "behaviour degraded (WildGuard 0.402 / 0.476 / 0.585 against 0.866 / 0.890 "
                  "/ 0.927) -- and steerable layers separate perfectly, 0 in every attack "
                  "against 15-17 in every control. F5 HOLDS IN ONLY 2 OF 3: the attacked "
                  "norm-matched re-fit is +0.21 / -0.80 / -0.22, i.e. about ZERO, where OLMo "
                  "2's was clearly negative (-0.60 / -1.41 / -3.38). The attack renders "
                  "Gemma's coupling INERT and OLMo 2's REVERSED; the pre-registered threshold "
                  "was written to OLMo 2's severity and is NOT redefined.",
        evidence=(
            Evidence("p1e7f_ANALYSIS", "p1e7f_check.py", "P1-E7f",
                     "F1-F5 per matched seed at dose 1500, plus the endpoint geometry, "
                     "steerable-layer counts and both judges' rates"),
        ),
        controls=(
            Control("family usable at all (Gate 0)", "reading a null result out of a model "
                    "that never had a steerable direction", True,
                    "2026-09-24: pre-registered gate passed on all four criteria -- l*=23, 20 "
                    "steerable layers of 34 unpruned, probe 1.000 with L0=0.500, baseline "
                    "refusal 1.000; peak ablation 22.87 vs norm-matched random 0.222.",
                    script="run_stage.py"),
            Control("control arm valid on this family (Gate 1)", "Tulu-2's failure mode: "
                    "safety-PRESERVING controls losing coupling too, leaving no contrast",
                    True, "2026-09-24: pre-registered gate run on control seed 1 BEFORE any "
                    "attack. Re-fit +5.87 and norm-matched +2.65 at dose 1500, both > 0.",
                    script="dose_response.py"),
            Control("seed 1's failure pinned against quiet rescue", "a later edit relaxing "
                    "F5 until the family passes", True,
                    "2026-09-25: the rule is p1e7f_check.pair_verdict, a pure function; "
                    "smoke_test drives it with seed 1's MEASURED numbers and asserts F5 is "
                    "the only criterion that fails, so relaxing F5 breaks the test suite.",
                    script="p1e7f_check.py"),
            Control("endpoint judge disagreements read", "reporting a behavioural rate whose "
                    "substring/WildGuard gap has not been inspected", True,
                    "2026-09-24: all 14 of seed 1's endpoint disagreements read against their "
                    "prompts -- 14/14 genuine refusals the substring judge misses. F4 holds "
                    "under BOTH judges. Seeds 2-3 (12, 15) not individually read.",
                    script="judge_wildguard.py"),
            Control("third and fourth families", "a two-family result presented as general",
                    False, "OPEN: OLMo-3-7B (Reblitz's) and Llama-3-8B-Instruct (Joad's) "
                    "would test whether 'inert vs reversed' tracks anything identifiable.",
                    optional=True),
        ),
        depends_on=("P1-E7r",),
        falsifier="Any seed where the attack's norm-matched re-fit is >= 0 while its control's "
                  "is > 0. IT FIRED, on seed 1 (+0.21 against +2.65), which is why this claim "
                  "is PARTIAL rather than confirmed.",
        note="Layer 2. Second family chosen AFTER the pre-registration literature check found "
             "Guo et al. (Findings EMNLP 2026) already own Llama-3.1 + Alpaca for this "
             "framing. Seeds 2-3 ran at doses 0 and 1500 only (P1 plan amendment 2, written "
             "before any attack run); F1-F5 all test the endpoint.",
    ),
    Claim(
        id="B1", layer=3,
        statement="RESOLVED 2026-09-22, REGENERATED BY SCRIPT 2026-09-23. Directional ablation "
                  "removes the INABILITY register and what survives is the model's FALLBACK "
                  "register, so ablation sufficiency tracks fallback capacity: Zephyr-DPO "
                  "100.0% (30 -> 0) > Tulu-2 70.0% (100 -> 30, identity 20 -> 21 untouched) > "
                  "OLMo 2 50.8% (130 -> 64; inability 128 -> 0, normative 2 -> 64), WildGuard, "
                  "128 tokens, the pre-registered ordering. Tulu-2 vs OLMo 2 overlaps at 95%: an "
                  "ordering, not a trend.",
        evidence=(
            Evidence("b1_registers_ANALYSIS", "b1_registers.py", "B1",
                     "regenerates every rate, count and register cell of the hand-written "
                     "b1_three_family_128 table from stored completions and WildGuard verdicts, "
                     "and fails loudly on any difference"),
        ),
        controls=(
            Control("matched generation length", "comparing a 48-token family against "
                    "128-token ones (48 tokens inflates refusal, O-139)", True,
                    "2026-09-22: all three families at 128 tokens.", script="b1_registers.py"),
            Control("labeller residuals read, not assumed", "counting refusals the labeller "
                    "misfiled as 'compliance' into a register without reading them", True,
                    "2026-09-23: 24 WildGuard refusals are stance_of v1 'compliance' (O-184 "
                    "gap). 18 were in the 48-token OLMo 2 hand audit; the 6 never read were "
                    "read (b1_residual_audit.json): all genuine normative refusals. The script "
                    "refuses to count an unaudited residual.", script="b1_registers.py"),
            Control("families beyond our three", "fallback capacity being a property of "
                    "Zephyr/Tulu-2/OLMo 2 rather than of alignment recipes", False,
                    "OPEN: Llama-3.1-8B, Gemma-2-9B (Joad's models), OLMo-3-7B (Reblitz's) -- "
                    "the claim map's B-session.", optional=True),
            Control("human register labels", "the register taxonomy being the authors' "
                    "reading", False, "OPEN: blind two-annotator labels (claim map, A3).",
                    optional=True),
        ),
        depends_on=("A2",),
        falsifier="Ablation removes all registers proportionally (no survivor pattern), or a "
                  "family with a large non-inability baseline register loses it to ablation. "
                  "Did not fire: Tulu-2's identity refusals go 20 -> 21.",
        note="The frozen JSON's confidence intervals record no method; b1_registers.py reports "
             "a paired prompt bootstrap and a Wilson interval on survival counts, labelled.",
    ),
    Claim(
        id="D1", layer=3,
